...
//    frame->height = data.height;
    int channels;
    unsigned char *data = stbi_load([pngFilePath UTF8String], &(frame->width), &(frame->height), &channels, STBI_rgb_alpha);
    int expectLength = frame->width * frame->height * 4;
...
//    int pixelsLength = MIN(expectLength, data.size);
    memcpy(pixels, (byte*) data, expectLength);
    frame->pixels = pixels;
